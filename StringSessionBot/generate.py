...
async def generate_session(bot, msg, telethon=False):
    ...
    phone_number_msg = await bot.ask(user_id, '- الان يرجى ارسال `PHONE_NUMBER` مع رمز الدوله . \nمثال : `+964xxxxxxx`', filters=filters.text)
    if await cancelled(api_id_msg):
        return
    ...
    try:
        ...
        await msg.reply('`API_ID` and `API_HASH` combination - أدخلت كلمة مرور غير صالحة. يرجى البدء في إنشاء الجلسة مرة أخرى.', reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return
    except (PhoneNumberInvalid, PhoneNumberInvalidError):
        await msg.reply('`PHONE_NUMBER` - أدخلت كلمة مرور غير صالحة. يرجى البدء في إنشاء الجلسة مرة أخرى.', reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return
    try:
    ...
    try:
        ...
        await msg.reply('OTP - أدخلت كلمة مرور غير صالحة. يرجى البدء في إنشاء الجلسة مرة أخرى.', reply_markup=InlineKeyboardMarkup(Data.generate_button))
        return
    ...
    text = "**{} STRING SESSION** \n\n`{}` \n\من قبل  @SANFOORA1".format("TELETHON" if telethon else "PYROGRAM", string_session)
    try:
        await client.send_message("me", text)
    except KeyError:
        pass
    await client.disconnect()
    await phone_code_msg.reply("Successfully generated {} string session. \n\nPlease check your saved messages! \n\nBy @SANFOORA1".format("telethon" if telethon else "pyrogram"))
